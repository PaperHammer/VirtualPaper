    ...
    public partial class EditConfigViewModel : ObservableObject {
        ...
        internal async Task InitContentAsync() {
            ...
            await loadingCtx.RunAsync(
                operation: async token => {
                    try {
                        var configData = await JsonSaver.LoadAsync<AvailableProjectTemplate>(_configPath, AvailableDraftTemplateContext.Default);
                        if (configData != null) {
                        ...
                        ArcLog.GetLogger<EditConfigViewModel>().Error(ex);
        ...
        public Task OnNextStepClickedAsync() {
            ...
            if (IsFromWorkSpaceForAddProj) {
                EditConfigTCS?.TrySetResult(preData);
            }
            else {
                _navigateComponent?.GetPaylaod()?.Set(NaviPayloadKey.Project, preData);
                _navigateComponent?.NavigateByState(EditPanelState.WorkSpace);
            }
        ...
        public Task OnPreviousStepClickedAsync() {
            _navigateComponent?.NavigateByState(EditPanelState.GetStart);
