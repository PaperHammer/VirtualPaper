...
namespace Workloads.Creation.StaticImg.Utils {
    ...
    //    internal static PathGeometry CreatePathGeometry(List<ArcPoint> points) {
    ...
    //    internal static ArcPoint? FormatPoint(Point? point, int digit) {
    //        return point == null ? null : new ArcPoint((float)Math.Round(point.Value.X, digit), (float)Math.Round(point.Value.Y, digit));
    //    }
    //}
}
