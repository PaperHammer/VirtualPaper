...
using VirtualPaper.Common.Utils.Buffers;
...
namespace Workloads.Creation.StaticImg.Models.ToolItems {
    partial class PaintBrushTool2(InkCanvasConfigData data) : Tool, IDisposable {
        ...
        private void DrawStroke(CanvasDrawingSession ds) {
            ...
            var brush = GetOrCreateBrush(size, _blendedColor);

            // 计算当前帧的脏矩形
            var currentDirtyRect = CalculateDirtyRect(_currentStroke, size);

            // 合并前后两帧的脏矩形（解决边缘残留问题）
            var combinedDirtyRect = CombineDirtyRects(currentDirtyRect, _lastDirtyRect);
            _lastDirtyRect = currentDirtyRect;

            // 使用图层限制绘制区域
            using (ds.CreateLayer(1.0f, combinedDirtyRect)) {
                // 单点模式
                if (_currentStroke.Count == 1) {
                    var point = _currentStroke[0];
                    ds.DrawImage(brush, (float)(point.X - size / 2), (float)(point.Y - size / 2));
                    return;
                }
            ...
            // 将当前帧的点存入环形缓冲区
            foreach (var point in _currentStroke) {
                _drawingBuffer.TryEnqueue(point);
            }
        }

        // 新增方法：计算脏矩形
        private Rect CalculateDirtyRect(List<Point> points, int brushSize) {
            ...
            foreach (var point in points) {
                double halfSize = brushSize / 2.0;
                left = Math.Min(left, point.X - halfSize);
                top = Math.Min(top, point.Y - halfSize);
                right = Math.Max(right, point.X + halfSize);
                bottom = Math.Max(bottom, point.Y + halfSize);
            }

            // 添加15%的安全边距
            double margin = (right - left) * 0.15;
            return new Rect(
                left - margin,
                top - margin,
                (right - left) + 2 * margin,
        ...
        // 新增方法：合并脏矩形
        private Rect CombineDirtyRects(Rect current, Rect previous) {
            if (previous.IsEmpty) return current;
            if (current.IsEmpty) return previous;

            return new Rect(
                Math.Min(current.Left, previous.Left),
                Math.Min(current.Top, previous.Top),
                Math.Max(current.Right, previous.Right) - Math.Min(current.Left, previous.Left),
                Math.Max(current.Bottom, previous.Bottom) - Math.Min(current.Top, previous.Top));
        }

        // 修改后的结束绘制方法
        private void EndDrawing() {
            _isDrawing = false;
            //_drawingBuffer.Clear(); // 清空缓冲区
            _lastDirtyRect = Rect.Empty; // 重置脏矩形
        ...
        /// 获取或创建笔刷纹理（带缓存优化）
        /// </summary>
        /// <param name="size">笔刷直径(像素)</param>
        /// <param name="color">笔刷颜色(带透明度)</param>
        /// <returns>创建或缓存的笔刷纹理</returns>
        private CanvasBitmap GetOrCreateBrush(int size, Color color) {
            var brushKey = (size, color);

            // 从缓存获取
            if (_brushCache.TryGetValue(brushKey, out var cachedBrush)) {
                return cachedBrush;
            }

            // 创建新笔刷
            var newBrush = CreateBrushTexture(size, color);

            // 缓存并清理旧笔刷
            CacheBrush(brushKey, newBrush);
        ...
        /// 创建圆形笔刷纹理
        /// </summary>
        private CanvasBitmap CreateBrushTexture(int size, Color color) {
            using var renderTarget = new CanvasRenderTarget(
                MainPage.Instance.SharedDevice,
                size, size, data.Size.Dpi,
                Windows.Graphics.DirectX.DirectXPixelFormat.B8G8R8A8UIntNormalized,
                CanvasAlphaMode.Premultiplied);

            using (var ds = renderTarget.CreateDrawingSession()) {
                ds.Clear(Colors.Transparent);
                RenderBrushShape(ds, size, color);
            }
        ...
        /// 渲染笔刷形状（可扩展不同笔刷类型）
        /// </summary>
        private void RenderBrushShape(CanvasDrawingSession ds, int size, Color color) {
            // 基础圆形笔刷
            float radius = size / 2f;
            ds.FillCircle(radius, radius, radius, color);
        ...
        /// 管理笔刷缓存（自动清理旧缓存）
        /// </summary>
        private void CacheBrush((int, Color) key, CanvasBitmap brush) {
            _brushCache[key] = brush;

            // 清理最不常用的50%缓存
            if (_brushCache.Count > BrushCacheLimit) {
                var toRemove = _brushCache
                    .OrderByDescending(x => x.Key.Item1) // 按笔刷大小排序
                    .Skip(BrushCacheLimit / 2)
                    .ToList();
        ...
        // 新增字段
        private readonly KfifoBuffer<Point> _drawingBuffer = new(4096); // 环形缓冲区
        private Rect _lastDirtyRect = Rect.Empty; // 记录上一次的脏矩形
        private const int BrushCacheLimit = 20;
    }
}
